...
class PageAttributes extends \yii\base\BaseObject
{
    ...
    public function getElementAttributes($elementKey = null) {
        if(!isset($elementKey)) return $this->_attributes;
        return isset($this->_attributes[$elementKey]) ? $this->_attributes[$elementKey] : [];
    }
    ...
    public function getElementAttribute($elementKey, $attribute) {
        return isset($this->_attributes[$elementKey][$attribute]) ? $this->_attributes[$elementKey][$attribute] : null;
    }
    ...
    public function element($elementKey, $selector = null) {
        ...
        if(!isset($selector) && !isset($this->_elementSelectors[$elementKey])) $selector = $elementKey;
        if(isset($selector)) { $this->_elementSelectors[$elementKey] = $selector; }
        return $this;
    }
    ...
    public function setAttribute($attribute, $value = null) {
        if(!isset($this->_elementOnCursor)) return $this;
        if(isset($value)) {
    ...
    public function getAttributes() {
        if(!isset($this->_elementOnCursor)) return [];
        $attributes = $this->getElementAttributes($this->_elementOnCursor);
        return $attributes;
    }
    ...
    public function renderAttributes($elementKey = null, $clearEmpty = true) {
        ...
        if($clearEmpty) { $this->removeEmptyAttributes(); }
    ...
    public function getAttributesForJs() {
        ...
        foreach ($this->_attributes as $elementKey => $elementAttributes) {
            if(!isset($this->_elementSelectors[$elementKey]) || empty($elementAttributes)) continue;
    ...
    protected function removeEmptyAttributes($elementKey = null) {
        $elementKey = isset($elementKey) ? $elementKey : $this->_elementOnCursor;
        if(!isset($this->_attributes[$elementKey])) return;
        foreach ($this->_attributes[$elementKey] as $attribute => $value) {
            if(is_array($value)) $value = implode(' ', $value);
            if(trim($value) === "") unset($this->_attributes[$elementKey]);
        }
    }
    ...
    protected function initElement($force = false, $elementKey = null) {
        $elementKey = isset($elementKey) ? $elementKey : $this->_elementOnCursor;
        if(!isset($elementKey)) return $this;
